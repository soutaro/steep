...
class A<'x> <: B<'x>
  ...
  def self?.baz: -> any
    ...
    assert_equal [parse_method("-> String")], methods[:to_s]
    assert_equal [parse_method("-> any")], methods[:foo]
    assert_equal [parse_method("-> any")], methods[:baz]
    assert_equal [parse_method("-> A")], methods[:itself]
    assert_equal [parse_method("(String) -> any")], methods[:eval]
    assert_equal [parse_method("() -> String")], methods[:get]
  end
...
module Bar
  def fizz: -> String
    ...
    assert_equal [parse_method("-> A")], methods[:foo]
    assert_equal [parse_method("-> A.class")], methods[:bar]
    assert_equal [parse_method("(String) -> A")], methods[:new]
    assert_equal [parse_method("(Number) -> Number")], methods[:abs]
    assert_equal [parse_method("-> String")], methods[:fizz]
    assert_equal [parse_method("-> any")], methods[:hoge]
    assert_equal [parse_method("-> String")], methods[:huga]
  end
end
