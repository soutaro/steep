        ...
        Steep.logger.tagged("#response_to_completion") do
          Steep.measure "Generating response" do
            ...
            when target = project.target_for_inline_source_path(job.path) || project.target_for_source_path(job.path)
              ...
              provider = Services::CompletionProvider::Ruby.new(source_text: file.content, path: job.path, subtyping: subtyping, contracts: project.contracts, postconditions: project.postconditions, delegation_registry: project.delegation_registry, return_alias: project.return_alias_registry)
      ...
      def process_signature_help(job)
        Steep.logger.tagged("##{__method__}") do
          if target = project.target_for_inline_source_path(job.path) || project.target_for_source_path(job.path)
            ...
            provider = Services::SignatureHelpProvider.new(source: source, subtyping: subtyping, contracts: project.contracts, postconditions: project.postconditions, delegation_registry: project.delegation_registry, return_alias: project.return_alias_registry)
