    ...
    class TypeCheckService
      ...
      def update_and_check(changes:, assignment:, &block)
        ...
        signatures.each do |path|
          if assignment =~ path
            each_signature_target(path) do |target|
              validate_signature(path, target, &block)
            end
          end
        end
      ...
      def each_signature_target(path, &block)
        if block
          project.targets.each do |target|
            service = signature_services[target.name] or raise
            next unless target.possible_signature_file?(path) || service.env_rbs_paths.include?(path)

            yield target
          end
        else
          enum_for :each_signature_target, path
        end
      end

      def validate_signature(path, target, &block)
