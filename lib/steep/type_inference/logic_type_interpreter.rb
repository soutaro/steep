      ...
      def evaluate_union_method_call(node:, env:, receiver:, receiver_type:)
        ...
        receiver_type.types.each do |type|
          ...
          when AST::Types::Name::Base
            ancestors = factory.definition_builder.ancestor_builder.singleton_ancestors(type.name).ancestors
            ancestors.find do |ancestor|
              call_type.method_decls.find do |decls|
                if ancestor.name == decls.method_def.defined_in
                  return_type = factory.type(decls.method_def.type.type.return_type)
                  truthy, falsey = factory.partition_union(return_type)
                  truthy_types << type if truthy
                  falsy_types << type if falsey
                  true
                end
              end
            end
          when AST::Types::Nil, AST::Types::Literal
            ancestors = factory.definition_builder.ancestor_builder.singleton_ancestors(type.back_type.name).ancestors
            ancestors.find do |ancestor|
              call_type.method_decls.find do |decls|
                if ancestor.name == decls.method_def.defined_in
                  return_type = factory.type(decls.method_def.type.type.return_type)
